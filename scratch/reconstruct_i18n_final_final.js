...
// FULL HUNGARIAN SET (Cleaned and Verified)
const HU_STRINGS = {
  'nav.home': 'Kezdőlap',
...
function run() {
    // 1. Recover "English" from the first block
    const execSync = require('child_process').execSync;
    execSync('git restore js/i18n.js');
    let content = fs.readFileSync(path, 'utf8');

    // Simple extraction of English
...
`;
    ...
    allLangs.forEach((lang, idx) => {
        ...
        if (lang === 'en') {
            ...
            // Try to extract from current content if available
            ...
            if (match) {
                ...
                while ((p = pairRegex.exec(match[1])) !== null) {
                    // Filter out Polish contamination if it's not the Polish block
        ...
        allKeys.forEach(k => {
            ...
            finalPairs.push(`    '${k}': '${val.replace(/'/g, "\\'")}'`);
        });
...
`;
    ...
    console.log("Master Reconstruction FINAL-FINAL complete.");
}
