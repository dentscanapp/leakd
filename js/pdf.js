(function() {
  ...
  function generatePDF(subs, totals) {
    ...
    const html = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Leakd - ${esc(t('pdf.title'))}</title>
        <style>
      ...
      <body>
        <div class="header">
          ...
          <div class="meta">${esc(t('pdf.generatedBy', { date: new Date().toLocaleDateString(lang) }))}</div>
        </div>
        
        <div class="summary">
          <div class="summary-box">
            <div class="summary-label">${esc(t('pdf.monthlyTotal'))}</div>
            <div class="summary-val">${formatPrice(totals.monthly)}</div>
          </div>
          <div class="summary-box">
            <div class="summary-label">${esc(t('pdf.yearlyTotal'))}</div>
            <div class="summary-val">${formatPrice(totals.yearly)}</div>
          </div>
          <div class="summary-box">
            <div class="summary-label">${esc(t('pdf.activeCount'))}</div>
        ...
        <table>
          <thead>
            <tr>
              <th>${esc(t('pdf.colService'))}</th>
              <th>${esc(t('pdf.colCategory'))}</th>
              <th>${esc(t('pdf.colBilling'))}</th>
              <th class="price-col">${esc(t('pdf.colAmount'))}</th>
            </tr>
          </thead>
          <tbody>
            ${activeList.map(s => {
